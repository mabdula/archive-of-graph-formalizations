...
definition tutte_matrix:: " ('a set => 'b ) \<Rightarrow> (('b::comm_ring_1, 'a) vec, 'a) vec" where
  "tutte_matrix x = (\<chi> i j. if (i, j) \<in> oriented_edges  then x {i, j}
                            else 
                                (if (j, i) \<in> oriented_edges then (- x {i, j}) 
                                 else 0 ))"

lemma in_oriented:
  assumes "(i, j) \<in> oriented_edges"
  shows "tutte_matrix x $i$j = x {i, j}" 
...
lemma tutte_matrix_det:
  "det (tutte_matrix x) =  sum (\<lambda>p. of_int (sign p) *
     prod (\<lambda>i. (tutte_matrix x)$i$p i) (UNIV :: 'a set))
...
definition nonzero_perms :: "('a set => 'b::comm_ring_1 ) \<Rightarrow> ('a \<Rightarrow> 'a) set "where
  "nonzero_perms x = {p. p permutes (UNIV :: 'a set) \<and> 
         prod (\<lambda>i. (tutte_matrix x)$i$p i) (UNIV :: 'a set) \<noteq> 0}"
...
proof
  ...
  have "p permutes UNIV  \<and> 
         prod (\<lambda>i. (tutte_matrix x)$i$p i) UNIV \<noteq> 0"
    using assms unfolding nonzero_perms_def by auto
  then have "prod (\<lambda>i. (tutte_matrix x)$i$p i) UNIV \<noteq> 0" by auto
  have "\<forall>i.  (tutte_matrix x)$i$p i \<noteq> 0" 
  proof
    fix i
    ...
    proof(rule ccontr)
      ...
      then have "prod (\<lambda>i. (tutte_matrix x)$i$p i) UNIV  = 0"
...
proof(cases "(i, (p i)) \<in> oriented_edges")
  case True
  then have "(tutte_matrix x)$i$p i = x {i, p i}" using in_oriented 
    by blast
  have "((p i), i) \<notin> oriented_edges" 
    using True order_less_asym oriented_edges_def by auto
  then show ?thesis 
    by (simp add: True \<open>local.tutte_matrix x $ i $ p i = x {i, (p i)}\<close> insert_commute rev_in_oriented)
next
  case False
  then have "(i, (p i)) \<notin> oriented_edges" 
    by simp
  ...
  proof(cases "(p i,  i) \<in> oriented_edges")
    case True
    then have "(tutte_matrix x)$i$p i = - x {i, p i}" using rev_in_oriented 
      by blast
    then show ?thesis 
      by (simp add: True \<open>local.tutte_matrix x $ i $ p i = - x {i, p i}\<close> in_oriented insert_commute)
  next
    case False
...
lemma rev_product_is_minus:
  assumes "p permutes (UNIV::'a set)"
  assumes "\<exists>C \<in> connected_components (u_edges p). odd (card C) "
  shows " prod (\<lambda>i. (tutte_matrix x)$i$p i) (UNIV :: 'a set) = 
          -  prod (\<lambda>i. (tutte_matrix x)$i$(rev_p p) i) (UNIV :: 'a set)" 
proof -
  let ?A = "set (support p  (least_odd p))"
  let ?h = "(\<lambda>i. (tutte_matrix x)$i$p i)" 
  let ?g = "(\<lambda>i. (tutte_matrix x)$i$(rev_p p) i)" 

  have "prod (\<lambda>i. (tutte_matrix x)$i$p i) UNIV = 
      prod (\<lambda>i. (tutte_matrix x)$i$p i) ?A *  prod (\<lambda>i. (tutte_matrix x)$i$p i) (UNIV - ?A)"
    by (metis (no_types, lifting) finite_class.finite_UNIV mult.commute prod.subset_diff top_greatest)

  have "prod (\<lambda>i. (tutte_matrix x)$i$(rev_p p) i) UNIV = 
      prod (\<lambda>i. (tutte_matrix x)$i$(rev_p p) i) ?A *  prod (\<lambda>i. (tutte_matrix x)$i$(rev_p p) i) (UNIV - ?A)"
    by (metis (no_types, lifting) finite_class.finite_UNIV mult.commute prod.subset_diff top_greatest)
  ...
  then have "prod ?h ?A = - prod (?g \<circ> (on_odd p)) ?A" using reverse_for_each_product 
    by (metis (no_types, lifting) \<open>odd (card ?A)\<close>)
  ...
  then have "prod ?h ?A = -  prod ?g ?A " 
    using \<open>(\<Prod>i\<in>set (support p (least_odd p)). local.tutte_matrix x $ i $ p i) = - prod ((\<lambda>i. local.tutte_matrix x $ i $ rev_p p i) \<circ> on_odd p) (set (support p (least_odd p)))\<close> by presburger
  then show ?thesis 
    using \<open>(\<Prod>i\<in>UNIV - set (support p (least_odd p)). local.tutte_matrix x $ i $ p i) = (\<Prod>i\<in>UNIV - set (support p (least_odd p)). local.tutte_matrix x $ i $ rev_p p i)\<close> \<open>(\<Prod>i\<in>UNIV. local.tutte_matrix x $ i $ p i) = (\<Prod>i\<in>set (support p (least_odd p)). local.tutte_matrix x $ i $ p i) * (\<Prod>i\<in>UNIV - set (support p (least_odd p)). local.tutte_matrix x $ i $ p i)\<close> \<open>(\<Prod>i\<in>UNIV. local.tutte_matrix x $ i $ rev_p p i) = (\<Prod>i\<in>set (support p (least_odd p)). local.tutte_matrix x $ i $ rev_p p i) * (\<Prod>i\<in>UNIV - set (support p (least_odd p)). local.tutte_matrix x $ i $ rev_p p i)\<close> mult_minus_left by auto
qed
...
lemma rev_opposite_value:
  ...
  shows "(\<lambda>p. of_int (sign p) *
     prod (\<lambda>i. (tutte_matrix x)$i$p i) UNIV) p = 
 - (\<lambda>p. of_int (sign p) *
     prod (\<lambda>i. (tutte_matrix x)$i$p i) UNIV) (rev_p p)" (is  " ?g  p = - ?g (rev_p p)")

proof -
  have "of_int (sign p) = of_int (sign (rev_p p))" 
    using assms(1) rev_same_sign by blast
  have " prod (\<lambda>i. (tutte_matrix x)$i$p i) UNIV =
  -  prod (\<lambda>i. (tutte_matrix x)$i$(rev_p p) i) UNIV"
...
proof -
  ...
  have " prod (\<lambda>i. (tutte_matrix x)$i$p i) UNIV \<noteq> 0"
    using assms unfolding nonzero_perms_def  
    by force
  then have " prod (\<lambda>i. (tutte_matrix x)$i$(rev_p p) i) UNIV \<noteq> 0"
...
proof -
  ...
  have "det (tutte_matrix x) =  sum ?g {p. p permutes (UNIV :: 'a set)}" 
  ...
  proof
    ...
    proof
      ...
      have "odd (least_power a i)" 
        using \<open>a \<in> nonzero_perms x\<close> \<open>a permutes UNIV\<close> \<open>i = (least_odd a)\<close> assms tutte_matrix.least_odd_is_odd tutte_matrix_axioms by blast
...
lemma no_perfect_matching_zero_det:
  ...
  shows "det (tutte_matrix x) = 0" 
proof(cases "\<exists> p \<in> nonzero_perms x. \<forall>C \<in> connected_components (u_edges p). even (card C)")
  case True
  then obtain p where "p \<in> nonzero_perms x \<and> (\<forall>C \<in> connected_components (u_edges p). even (card C))" 
    by auto
  ...
  have "u_edges p \<subseteq> E" 
    by (meson \<open>p \<in> nonzero_perms x \<and> (\<forall>C\<in>connected_components (u_edges p). even (card C))\<close> wqewqe1)
  have "Vs (u_edges p) = Vs E" 
    by (meson \<open>p \<in> nonzero_perms x \<and> (\<forall>C\<in>connected_components (u_edges p). even (card C))\<close> perm_verts_eq_all)
...
next
  case False
  then have "\<forall>p \<in> nonzero_perms x. \<exists>C \<in> connected_components (u_edges p). odd (card C)" 
    by blast
