...
proof -
  ...
  then have "graph_invar (component_edges (u_edges p) C)" 
    by (metis (no_types, lifting) Berge.component_edges_subset \<open>C \<subseteq> Vs (u_edges p)\<close>
        assms(1) finite_subset insert_subset mk_disjoint_insert u_edges_is_graph)
  then  have " perfect_matching (component_edges (u_edges p) C) ?m_edges"
    unfolding perfect_matching_def 
    using 8 \<open>Vs ?m_edges = C\<close> \<open>matching ?m_edges\<close> 9 by blast
...
proof(cases "\<exists> p \<in> nonzero_perms. \<forall>C \<in> connected_components (u_edges p). even (card C)")
  ...
  then have "perfect_matching E M" 
    using M unfolding perfect_matching_def using \<open>u_edges p \<subseteq> E\<close> graph by auto
  then show ?thesis 
    using assms by auto
next
  case False
...
proof -
  ...
  then have "prod (\<lambda>i. (tutte_matrix)$i$p i) UNIV \<noteq> 0" 
    using all_edges_in_E_prod_nonzero[of p] \<open>\<forall>i. {i, p i} \<in> E\<close> assms(1) by fastforce
  then show "p \<in> nonzero_perms" 
...
lemma perfect_matching_nonzero_det:
  ...
  shows "det (tutte_matrix) \<noteq> 0"
proof -
  obtain M where M:"perfect_matching E M" 
    using assms by blast
  ...
  proof
    ...
    then obtain j where j: "e = {i, j}" 
      using `graph_invar M` 
      by (metis (no_types, lifting) e insert_commute insert_iff singletonD)
  ...
  have "?f \<in> nonzero_perms"
    ...
    using \<open>?f permutes UNIV\<close>  \<open>u_edges ?f = M\<close>  by blast
